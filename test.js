...
async function run() {
  ...
  try {
    ...
    const ing1 = await Ingredient.create({
      name: `Flour ${suffix}`,
      description: "Wheat flour",
    });
    const ing2 = await Ingredient.create({
      name: `Sugar ${suffix}`,
    ...
    const riRows = await RecipeIngredient.findAll({
      where: { recipeId: recipe.id },
    });
    console.log(
      "recipe_ingredients rows:",
      riRows.map((r) => ({ ingredientId: r.ingredientId, measure: r.measure }))
    );
    ...
    console.log(
      "Followers of userA emails:",
      followersOfA.map((u) => u.email)
    );
    ...
    await RecipeIngredient.destroy({
      where: { recipeId: recipe.id, ingredientId: ing2.id },
    });
